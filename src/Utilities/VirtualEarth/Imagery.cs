namespace JosephGuadagno.Utilities.VirtualEarth
{
    ...
    public static class Imagery
    {
        ...
        public static string GetMapUri(string appId, double latitude, double longitude, int zoom, string mapStyle,
            int width, int height, Pushpin[] pushpins)
        {
        ...
        public static string GetMapUri(string appId, int zoom, string mapStyle, int width, int height,
            Pushpin[] pushpins)
        {
        ...
        {
            if (pushpins.Length > 0)
                return GetMapUri(appId, pushpins[0].Location.Latitude, pushpins[0].Location.Longitude, _defaultZoom,
                    _defaultMapStyle, _defaultWidth, _defaultHeight, pushpins);
            return GetMapUri(appId, _latitude, _longitude, _defaultZoom, _defaultMapStyle, _defaultWidth, _defaultHeight,
                pushpins);
        }

        public static string GetMapUri(string appId, double latitude, double longitude)
        {
            return GetMapUri(appId, latitude, longitude, _defaultZoom, _defaultMapStyle, _defaultWidth, _defaultHeight,
                null);
        }
        ...
        public static string GetMapUri(string appId, double latitude, double longitude, int width, int height,
            Pushpin[] pushpins)
        {
